...
const CLASS_ID = Components.ID("46750150-1378-11de-8c30-0800200c9a66");
const CLASS_NAME = "Listens for measurements and acquires location using 3 protocols: HELD, Skyhook, and Gears";
const CONTRACT_ID = "@geopriv.dreamhosters.com/unitylocationprovider;1";

// This is your constructor.
// You can do stuff here.
function UnityLocationProvider() {
	// you can cheat and use this
	// while testing without
	// writing your own interface
	this.wrappedJSObject = this;

	// Set up a re-usable request objects
	this._heldReq = this._makeNewHELDRequest();
	this._skyhookReq = this._makeNewSkyhookRequest();
	this._gearsReq = this._makeNewGearsRequest();
}

// This is the implementation of your component.
UnityLocationProvider.prototype = {
	// for nsISupports
	...
	// HELD server URI
	// No action on getting	
	get heldURI() { return this._heldURI; },
	set heldURI(uri) {
		// If the URI changes, we need a new request
		this._heldURI = uri;
		this._heldReq = this._makeNewHELDRequest();
	},
	...
	_heldURI: null,		// HELD server that we'll query for location
	_heldReq: null,		// HELD request to be used to get location
	_skyhookReq: null,	// Skyhook request to be used to get location
	_gearsReq: null,	// Gears request to be used to get location
	...
	// Make a new HELDRequest using the currently-configured parameters
	_makeNewHELDRequest : function() {
		return new HELDLocationRequest(
			this._heldURI,
			null,
			function(code, text) {
				this._log("Error: code=[HELD-"+code+"] text=["+text+"]");
			}
		);
	},

	// Make a new SkyhookRequest using the currently-configured parameters
	_makeNewSkyhookRequest : function() {
		var SLP = this;
		return new SkyhookLocationRequest(
			function(code, text) {
				SLP._log("Error: code=[Skyhook-"+code+"] text=["+text+"]");
			}
		);
	},
	...
	_makeNewGearsRequest : function() {
		...
		return new GearsLocationRequest(
			function(code, text) {
				SLP._log("Error: code=[Gears-"+code+"] text=["+text+"]");
			}
		);
	},
	...
	_handleMeasurementRequest: function(request, response, parent) {
		...
		if (!meas) { 
			response.setStatusLine( request.httpVersion, 400, "Bad request: need Unity Protocol" );
			// Do not update watchers or cache
			return;
		}

		// Make location requests from all three services
		// 1. HELD
		parent._heldReq.meas = meas;
		var loc_held = parent._heldReq.send();
		// 2. Skyhook
		parent._skyhookReq.meas = meas;
		var loc_skyhook = null
		if (meas.wifi_towers) lok_skyhook = parent._skyhookReq.send(); // Only send to Skyhook if we've got wifi readings
		// 3. Gears
		parent._gearsReq.meas = requestBody;
		var loc_gears = parent._gearsReq.send();
		
		// Fail if we don't get any location back
		if (!loc_held && !loc_skyhook && !loc_gears) {
			response.setStatusLine( request.httpVersion, 404, "Unable to determine location" );
			// Do not update watchers or cache
			return;
		}
				
		// Assemble a location object out of whatever location we got
		var loc = {};
		// 1. Geodetic: Take the minimum uncertainty (preferring any over a point)
		// Take HELD as a starting point if it's there
		if (loc_held && loc_held.geodetic) 
			loc.geodetic = loc_held.geodetic;
		// Else, If we've got Skyhook and it's better (either smaller or zero->nonzero)
		if (loc_skyhook && loc_skyhook.geodetic) {
			if ((!loc.geodetic) || 
				((loc.geodetic.getUncertainty() > loc_skyhook.geodetic.getUncertainty()) && (loc_skyhook.geodetic.getUncertainty() != 0)) ||
				((loc.geodetic.getUncertainty() == 0) && (loc_skyhook.geodetic.getUncertainty() != 0))) {
				loc.geodetic = loc_skyhook.geodetic;
			}
		}
		// Else, If we've got Skyhook and it's better (either smaller or zero->nonzero)
		if (loc_skyhook && loc_skyhook.geodetic) {
			if ((!loc.geodetic) || 
				((loc.geodetic.getUncertainty() > loc_gears.geodetic.getUncertainty()) && (loc_gears.geodetic.getUncertainty() != 0)) ||
				((loc.geodetic.getUncertainty() == 0) && (loc_gears.geodetic.getUncertainty() != 0))) {
				loc.geodetic = loc_gears.geodetic;
			}
		}
		
		// 2. Civic: Take the one that has the most precise info in the following sequence
		// country > A1 > A2 > A3 > A4 > A4 > A6 > PC > RD > HNO > UNIT > BLD > NAM
		var caTypes = ["country", "A1", "A2", "A3", "A4", "A5", "A5", "A6", 
						"PC", "RD", "HNO", "UNIT", "BLD", "NAM"];
		var heldScore=0, skyhookScore=0, gearsScore=0;
		function uniquemax(a,b,c) { 
		...
		// Work from most precise to least, looking for a tiebreaker 
		var umax = null;
		for (var i=caTypes.length-1; i>=0; --i) {
			// Increment the score if you have a CAtype
			if (loc_held && loc_held.civic && loc_held.civic[caTypes[i]]) heldScore++;
			if (loc_skyhook && loc_skyhook.civic && loc_skyhook.civic[caTypes[i]]) skyhookScore++;
			if (loc_gears && loc_gears.civic && loc_gears.civic[caTypes[i]]) gearsScore++;
			// Check for a unique maximum and break if so
			umax = uniquemax(heldScore, skyhookScore, gearsScore);
			if (umax) break;
		}
		if (umax) {
			// If there's a clear winner, take that one 
			// (Has to be must be non-null, or else scores=0,0,0)
			if (heldScore==umax) loc.civic = loc_held.civic;
			else if (skyhookScore==umax) loc.civic = loc_skyhook.civic;
			else if (gearsScore==umax) loc.civic = loc_gears.civic;
		} else {
			// Else take the first with the highest score
			umax = Math.max(Math.max(heldScore,skyhookScore),gearsScore);
			// If there's a clear winner, take that one 
			// (Has to be must be non-null, or else scores=0,0,0)
			if (heldScore==umax && loc_held) loc.civic = loc_held.civic;
			else if (skyhookScore==umax && loc_skyhook) loc.civic = loc_skyhook.civic;
			else if (gearsScore==umax && loc_gears) loc.civic = loc_gears.civic;
			else loc.civic = null;
		}
		
		// 3. Rules: Take the one from HELD or nothing
		if (loc_held) loc.rules = loc_held.rules;
		else loc.rules = null;
		
		// 4. URIs: Take the ones from HELD or nothing
		if (loc_held) loc.locationURIs = loc_held.locationURIs;
		else loc.locationURIs = null; 

		// 5. Timestamp: Just take the first non-null one we find
		if (loc_held && loc_held.timestamp) loc.timestamp = loc_held.timestamp;
		else if (loc_skyhook && loc_skyhook.timestamp) loc.timestamp = loc_skyhook.timestamp;
		else if (loc_gears && loc_gears.timestamp) loc.timestamp = loc_gears.timestamp;

		// Log this location to a central log server
		this._remoteLog(loc);
		...
		// Pull out centroid and uncertainty and pass them back to unity
	...
	// Send a logging message to the log server
	_remoteLog : function(loc) {
		var url = "http://geopriv.dreamhosters.com/cgi-bin/geolog.php?";
		url += "lat="+ loc.geodetic.getCentroid().latitude +"&";
		url += "lon="+ loc.geodetic.getCentroid().longitude +"&";
		url += "unc="+ loc.geodetic.getUncertainty() ;
	
		var req = getXMLHttpRequest();
                req.open("POST", url, true);
                req.send(loc.toSource());
	},
	...
	// Logging shortcut
	_log : function(msg) { Components.utils.reportError("HELP: "+msg); },
}
...
//
// Factory
var UnityLocationProviderFactory = {
	createInstance: function (aOuter, aIID)
	{
		if (aOuter != null)
			throw Components.results.NS_ERROR_NO_AGGREGATION;
		return (new UnityLocationProvider()).QueryInterface(aIID);
	}
};

// Module
var UnityLocationProviderModule = {
	...
	getClassObject: function(aCompMgr, aCID, aIID)
	{
		...
		if (aCID.equals(CLASS_ID))
			return UnityLocationProviderFactory;
...
function NSGetModule(aCompMgr, aFileSpec) { return UnityLocationProviderModule; }
...
//// Uncomment if using in an XPCOM context, e.g., in an extension
	return Components.classes["@mozilla.org/xmlextras/xmlhttprequest;1"]
		.createInstance(Components.interfaces.nsIXMLHttpRequest); 
////Uncomment for use within a window
...
HELDLocationRequest.prototype =
{
    ...
    {
        var query = '<locationRequest xmlns="urn:ietf:params:xml:ns:geopriv:held">';
        ...
        if (this.ip)
        {
            query += '<deviceIdentity xmlns="urn:ietf:params:xml:ns:geopriv:held:id">';
            query += '<uri>ip:IPv4+' + this.ip + '</uri>';
            query += '</deviceIdentity>';
        }
        if (this.meas)
        {
        	query += '<measurements xmlns="urn:ietf:params:xml:ns:geopriv:lm">'
        	if (this.meas.wifi_towers) {
        		query += '<wifi xmlns="urn:ietf:params:xml:ns:geopriv:lm:wifi">';
				for (var i=0; i < this.meas.wifi_towers.length; ++i) {
					var ap = '';
			        ap += '<neighbourWap>';
			        if (this.meas.wifi_towers[i].ssid)
			        	ap += '<ssid>'+ this.meas.wifi_towers[i].ssid +'</ssid>';
			        ap += '<bssid>'+ this.meas.wifi_towers[i].mac_address +'</bssid>';
			        ap += '<rssi dBm="false">'+ this.meas.wifi_towers[i].signal_strength +'</rssi>';
			        ap += '</neighbourWap>';
					query += ap;
				}
				query += '</wifi>';
        ...
        // TODO: Add other types of measurements
    ...
    ready: function()
    {
        if (this.request.readyState == 4)
        {
            if ((this.request.status == 200 || this.request.status == 0)
                    && this.request.responseXML.documentElement) // TODO: remove 0
            {
...
2. ... It uses an XPCOM channel to do the dirty work (Skyhook chokes on XHRs)
...
req = new SkyhookLocationRequest(errorCallback)
loc = req.send();
...
var NS_SKYHOOK = "http://skyhookwireless.com/wps/2005";

function SkyhookLocationRequest(_errorCallback)
{
    this.errorCallback = _errorCallback;
}

SkyhookLocationRequest.prototype =
{
	url: "https://api.skyhookwireless.com/wps2/location",
	username: "beta", // Pretend to be Loki
	realm: "js.loki.com",
	meas: null,	// Gears API request object format

    constructQuery: function()
    {
        var query = '<LocationRQ xmlns="http://skyhookwireless.com/wps/2005" version="2.6" street-address-lookup="full">';

		if (this.username && this.realm) {
			query += '<authentication version="2.0"><simple>';
			query += '<username>'+ this.username +'</username>';
			query += '<realm>'+ this.realm +'</realm>';			
			query += '</simple></authentication>'
		}

        if (this.meas && this.meas.wifi_towers)
        {
			for (var i=0; i < this.meas.wifi_towers.length; ++i) {
				var ap = "<access-point>";
				ap += "<mac>"+ this.meas.wifi_towers[i].mac_address.split("-").join("") +"</mac>";
				ap += "<signal-strength>"+ this.meas.wifi_towers[i].signal_strength  +"</signal-strength>";
				ap += "</access-point>";
				query += ap;
			}
        }
    ...
    process: function(xml)
    {
        ...
        // Find the first Element under the root
        var child = doc.firstChild;
	// Node.ELEMENT_NODE : 1
        while (child.nodeType != 1 && child.nextSibling) {
        	child = child.nextSibling;
        }
        var local = child.localName ? child.localName : child.baseName;
        
        // If we got an error, then the first child of <LocationRS> will be <error>
        // In that case, we call 
        if (local == 'error')
        {
            if (this.errorCallback)
            {
                this.errorCallback("skyhookError",
                                   child.textContent);
            }
        }
        else if (local == 'location')
        {
            try
            {
                var loc = Location.prototype.fromSkyhook(doc);
                if (this.callback)
                {
                    this.callback(loc);
                }
    ...
    send: function()
    {
		...
		uploadChannel.setUploadStream(postStream, "text/xml", -1);
		...
		var xml = parser.parseFromString(responseBody, "text/xml");
...
Location.prototype = {
    ...
    // Translates from Skyhook XML to PIDF-LO, then calls Location(PIDF-LO)
    fromSkyhook: function(xml) 
    {
    	// Locate all the relevant nodes
    	// XXX: Not quite accurate, because of the <address-line> element
    	var loc = findFirstElement(xml, 'http://skyhookwireless.com/wps/2005', 'location');
    	if (!loc) {
    		errorCallback("syntaxError", "No location present");
    		return;
    	}
    	var lat = findFirstElement(loc, 'http://skyhookwireless.com/wps/2005', 'latitude');
    	var lon = findFirstElement(loc, 'http://skyhookwireless.com/wps/2005', 'longitude');
    	var hpe = findFirstElement(loc, 'http://skyhookwireless.com/wps/2005', 'hpe');
    	var addr = findFirstElement(loc, 'http://skyhookwireless.com/wps/2005', 'street-address');
    	var country = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'country');
    	var A1 = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'state');
    	var A2 = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'county');
    	var A3 = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'city');
    	var PC = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'postal-code');
    	var HNO = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'street-number');
    	var RD = findFirstElement(addr, 'http://skyhookwireless.com/wps/2005', 'address-line');
    	...
    	if (lat && lon && hpe) 
    	{
    		...
    		pidf += '<gml:pos>'+ lat.textContent +' '+ lon.textContent +'</gml:pos>';
			pidf += '<gs:radius uom="urn:ogc:def:uom:EPSG::9001">'+ hpe.textContent +'</gs:radius>';
			pidf += '</gs:Circle>';
    	}
    	if (addr) 
    	{
    		pidf += '<ca:civicAddress>';
    		if (country) pidf += '<ca:country>'+ country.getAttribute("code") +'</ca:country>';
    		if (A1) pidf += '<ca:A1>'+ A1.getAttribute("code") +'</ca:A1>';
    		if (A2) pidf += '<ca:A2>'+ A2.textContent +'</ca:A2>';
    		if (A3) pidf += '<ca:A3>'+ A3.textContent +'</ca:A3>';
    		if (PC) pidf += '<ca:PC>'+ PC.textContent +'</ca:PC>';
    		if (HNO) pidf += '<ca:HNO>'+ HNO.textContent +'</ca:HNO>';
    		if (RD) pidf += '<ca:RD>'+ RD.textContent +'</ca:RD>';
    		pidf += '</ca:civicAddress>';
    	}
