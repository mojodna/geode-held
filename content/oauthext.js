...
OAuth.getUrl = function getUrl(method, url, consumerKey, consumerSecret, signatureMethod, token, tokenSecret, params) {
	var parameterMap = OAuth.getParams(method, url, consumerKey, consumerSecret, signatureMethod, token, tokenSecret, params);
	var keys = [];
	var paramString = "";
	for (var p in parameterMap) {
		if (typeof parameterMap[p] !== 'function') {
			var value = encodeURI(parameterMap[p]);
 ...
 */
OAuth.getParams = function getParams(method, url, consumerKey, consumerSecret, signatureMethod, token, tokenSecret, params) {
	if (tokenSecret === undefined) {
		tokenSecret = "";
	}
	var accessor = {consumerSecret : consumerSecret, tokenSecret : tokenSecret};
	var message = {action : url, method : method, parameters : []};
	OAuth.setParameter(message, "oauth_consumer_key", consumerKey);
	OAuth.setParameter(message, "oauth_signature_method", signatureMethod);
	OAuth.setParameter(message, "oauth_version", "1.0");
	if (token !== null && token !== "") {
		OAuth.setParameter(message, "oauth_token", token);
	}
